...
// export function main(project: string, tslintFile: string, basePath?: string, codegen?:
// CodegenExtension): Promise<any> {
        ...
        if (result.failureCount > 0) {
          ...
          if (replacements.length > 0) {
            ...
            replacements.forEach(r => {
              newSource =
                  newSource.substring(0, r.startPosition) + r.text + source.substring(r.endPosition);
            });
...
if (require.main === module) {
  ...
  // main(args.p || args.project || '.', args.t || args.tslint || 'tslint.json', args.basePath)
